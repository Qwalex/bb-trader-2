...
**`userbot` крашится в рантайме с `ModuleNotFoundError: No module named 'structlog'` (или любая другая зависимость)** → `uv sync` ставит зависимости в venv `/app/.venv`, а **Railway `startCommand: python …`** может вызвать **системный** интерпретатор, игнорируя `PATH`. Решение: в `apps/userbot/railway.json` задать `startCommand` с **абсолютным путём**: `/app/.venv/bin/python -m bb_userbot.main` (и в Dockerfile такой же `CMD`). Дополнительно в образе остаётся `ENV PATH="/app/.venv/bin:$PATH"`. Альтернатива — `uv run python -m bb_userbot.main` (медленнее старт).
